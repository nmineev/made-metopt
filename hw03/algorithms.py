...
def gd(filename, x_init, A, y, gamma, 
    ...
    #метод
    for it in tqdm.tqdm(range(S), desc="Train epochs:"):
...
def sgd_decr_stepsize(filename, x_init, A, y, gamma_schedule, 
    ...
    c = 1
    d = 0
    for it in tqdm.tqdm(range(int(S*m/batch_size)), desc="Train epochs:"):
        #ваш код здесь
        if d >= decr_period * c:
...
def svrg(filename, x_init, A, y, gamma, 
    ...
    #метод
    w = x
    for s in tqdm.tqdm(range(S), desc="Train epochs:"):
